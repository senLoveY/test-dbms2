...
const QUESTION_COUNTS = [5, 10, 15, 20, 30, 50, 100];
...
export function questionCountChoices(maxQuestions) {
  const max = Math.max(1, Number(maxQuestions) || 100);
  const presets = QUESTION_COUNTS.filter((count) => count < max);
  return [...new Set([...presets, max])];
}
...
export function normalizeRoomSettings(input = {}) {
  ...
  return {
    ...
    questionCount: clampInt(base.questionCount, 1, 100, 10),
